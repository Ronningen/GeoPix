...
from ...prompt_encoder.block.prompt_encoder import PositionEmbeddingRandom
...
class MaskDecoderMultiScale(nn.Module):
    ...
    def predict_masks(
        ...
        ) # self.iou_token.weight.shape=[1,256], self.mask_tokens.weight.shape=[4,256]. 总的[5, 256]
        output_tokens = output_tokens.unsqueeze(0).expand(
            sparse_prompt_embeddings.size(0), -1, -1
        ) # [1, 5, 256]
        # sparse_prompt_embeddings.shape = [1,1,256]
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1) # [1,6,256]
        ...
        tokens = tokens + level_embed # [num_objs,6,256]

        # image_embeddings: [1, C, H, W], tokens: [B, N, C]
        # dense_prompt_embeddings: [B, C, H, W]
        # Expand per-image data in batch direction to be per-mask
        if not image_embeddings.shape[0] == tokens.shape[0]:
            src = torch.repeat_interleave(image_embeddings, tokens.shape[0], dim=0)
        else:
            src = image_embeddings # the image is already batched # [1,256,32,32]
        # import pdb; pdb.set_trace()
        if level_num > 0:
            src = self.upsample_2x(src) # [1,256,32,32] -> [1,256,64,64]
            b, c, h, w = src.shape
        ...
        src = src + dense_prompt_embeddings #B, C, 32, 32。超参定死了C=256,在外面定死了B是1
        pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0) # [1,256,32,32]
        b, c, h, w = src.shape

        # Run the transformer
        hs, src = self.transformer[level_num](src, pos_src, tokens) # hs.shape=[1,6,256]; src.shape=[1,1024,256] 
        # nhs, nsrc = self.transformer[level_num](torch.randn([1,256,24,24]), torch.randn([1,256,24,24]), torch.randn([1, 6, 256])) # [1,576,256]
        iou_token_out = hs[:, 0, :] # [1,256]
        mask_tokens_out = hs[:, 1 : (1 + self.num_mask_tokens), :] # [1,4,256]

        # Upscale mask embeddings and predict masks using the mask tokens
        src = src.transpose(1, 2).view(b, c, h, w) # [1,256,32,32] #

        # if level_num == self.image_feature_scale_num-1:
        upscaled_embedding = self.output_upscaling(src) # [1,32,64,64]  dim:256->32, h,w:32->64
        ...
        # import pdb; pdb.set_trace()
        b, c, h, w = upscaled_embedding.shape # [1,32,64,64]
